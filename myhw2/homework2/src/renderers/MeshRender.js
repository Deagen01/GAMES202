...
class MeshRender {
	...
	draw(camera) {
		...
		//在这里赋值了？
		//一共有三个LT 这是lighting transport?
		//ii是表示啥有三个场景吗？
		for (var ii = 0; ii < 3; ++ii) {
			gl.enableVertexAttribArray(this.shader.program.attribs['aPrecomputeLT'] + ii);
			// 第一个参数this.shader.program.attribs['aPrecomputeLT'] + ii表示顶点属性数组的索引，与上面的代码相同。
			// 第二个参数3表示每个顶点属性的分量数量，这里是3个浮点数。
			// 第三个参数gl.FLOAT表示每个分量的数据类型是浮点数。
			// 第四个参数false表示是否归一化数据。
			// 第五个参数36表示每个顶点属性的字节大小，这里是36字节。
			// 第六个参数ii * 12表示顶点属性数组的偏移量，即从缓冲区的哪个位置开始读取数据，这里是每次迭代都会增加12字节（3个浮点数 * 4字节/浮点数）。
