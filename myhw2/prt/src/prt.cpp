        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    //采样次数是6*widht*height 积分变成累加
                    //对每一阶进行积分
                    double area = CalcArea(x,y,width,height);
                    for (int l =0; l<SHOrder;l++){
                        for (int m =-l;m<l+1;m++){
                            double sh_value = sh::EvalSH(l,m,dir.cast<double>().normalized());
                            SHCoeffiecents[l*(l+1)+m]+=(sh_value*Le*area);
                        }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        //m_LightCoeffs：构建一个3×9的矩阵，存放三个通道的系数
        //存放lighting部分的球谐系数
        ...
        //m_TransportSHCoeffs：构建一个9×顶点数的矩阵
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                else
                {
                    ...
                    //从sd生成对应方向的光线
                    //与场景求交
                    Ray3f ray(Point3f(v),wi.normalized());
                    double H = n.dot(wi);
                    //不与场景相交 sp被遮挡 返回H
